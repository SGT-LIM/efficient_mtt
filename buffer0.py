...
def main(args):
    ...
    for it in range(0, args.num_experts): #args.num_experts = 100
        ...
        lr_schedule = [args.train_epochs // 2 - 1] # lr_schedule = 26 -> 24

        lambda_values = np.linspace(0.5, 1, 5)

        for e in range(args.train_epochs): # args.train_epochs = 50

            if e < 5:
                current_lambda = lambda_values[e]
                teacher_optim = torch.optim.SGD(teacher_net.parameters(), lr=lr, momentum=args.mom, weight_decay=current_lambda) # optimizer_img for synthetic data
                teacher_optim.zero_grad()
            ...
            if e in lr_schedule and args.decay: #act here
                lr *= 0.5
                teacher_optim = torch.optim.SGD(teacher_net.parameters(), lr=lr, momentum=args.mom, weight_decay=current_lambda)
                teacher_optim.zero_grad()
